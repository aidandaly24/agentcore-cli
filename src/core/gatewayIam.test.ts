...
const OLD_POLICY_ENGINE_ARN =
  "arn:aws:bedrock-agentcore:us-west-2:123456789012:policy-engine/orders-old";
...
describe("GatewayClient managed execution role", () => {
  ...
  test("stages current and desired permissions around a Gateway update", async () => {
    const policyName = ExecutionRoleManager.generatedPolicyName("gateway", {
      accountId: ACCOUNT_ID,
      region: REGION,
      stableResourceKey: ROLE_NAME,
        ...
        JSON.stringify({
          ...
          Statement: [
            ...
            },
            {
              ...
              Resource: [OLD_POLICY_ENGINE_ARN],
            },
            {
              Effect: "Allow",
              Action: [
              ...
              Resource: [GATEWAY_ARN, OLD_POLICY_ENGINE_ARN],
            },
          ],
        }),
      ],
    ...
    const current: GetGatewayResponse = {
      ...
      roleArn: ROLE_ARN,
      authorizerType: "NONE",
      policyEngineConfiguration: { arn: OLD_POLICY_ENGINE_ARN, mode: "ENFORCE" },
    };
    const ready: GetGatewayResponse = {
      ...current,
      updatedAt: new Date("2026-08-12T00:01:00Z"),
      policyEngineConfiguration: { arn: POLICY_ENGINE_ARN, mode: "ENFORCE" },
    };
    const updating: UpdateGatewayResponse = {
      ...ready,
      status: "UPDATING",
    };
    let updated = false;
    const control = {
      send: async (
        command: GetGatewayCommand | ListGatewayTargetsCommand | UpdateGatewayCommand,
      ) => {
        events.push(command.constructor.name);
        ...
        updated = true;
        expect(policyPermissions(policies.get(policyName)!)).toEqual([
          `bedrock-agentcore:AuthorizeAction ${GATEWAY_ARN}`,
          `bedrock-agentcore:AuthorizeAction ${POLICY_ENGINE_ARN}`,
          `bedrock-agentcore:AuthorizeAction ${OLD_POLICY_ENGINE_ARN}`,
          `bedrock-agentcore:GetPolicyEngine ${POLICY_ENGINE_ARN}`,
          `bedrock-agentcore:GetPolicyEngine ${OLD_POLICY_ENGINE_ARN}`,
          `bedrock-agentcore:InvokeGateway ${GATEWAY_ARN}`,
          `bedrock-agentcore:PartiallyAuthorizeActions ${GATEWAY_ARN}`,
          `bedrock-agentcore:PartiallyAuthorizeActions ${POLICY_ENGINE_ARN}`,
          `bedrock-agentcore:PartiallyAuthorizeActions ${OLD_POLICY_ENGINE_ARN}`,
        ]);
        return updating;
      },
    } as unknown as BedrockAgentCoreControlClient;
    ...
    expect(events.indexOf("PutRolePolicyCommand")).toBeLessThan(
      events.indexOf("UpdateGatewayCommand"),
    );
    expect(policyPermissions(policies.get(policyName)!)).toEqual([
      `bedrock-agentcore:AuthorizeAction ${GATEWAY_ARN}`,
      `bedrock-agentcore:AuthorizeAction ${POLICY_ENGINE_ARN}`,
