...
describe("option help groups", () => {
  ...
  test("add harness renders its help groups in order with tags under configuration", async () => {
    const r = renderScreen("/agentcore/add/harness");
    await r.resize(100, 100);
    ...
    expect(frame.slice(positions[0], positions[1])).toContain("--tags");
    r.unmount();
  });
