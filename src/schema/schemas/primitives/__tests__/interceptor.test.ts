...
import { InterceptorSchema, resolveInterceptorEntrypoint } from '../interceptor';
...
describe('InterceptorSchema', () => {
  ...
  it('rejects a nodejs22.x runtime paired with a Python-style entrypoint', () => {
    const r = InterceptorSchema.safeParse({
      name: 'node-bad',
      gatewayName: 'my-gw',
      interceptionPoints: ['REQUEST'],
      config: {
        managed: { codeLocation: 'app/node-bad/', runtime: 'nodejs22.x', entrypoint: 'handler.lambda_handler' },
      },
    });
    expect(r.success).toBe(false);
    if (!r.success) {
      expect(r.error.issues.some(i => /Python handler.*nodejs22\.x/.test(i.message))).toBe(true);
    }
  });

  it('accepts a nodejs22.x runtime with a Node-style entrypoint', () => {
    const r = InterceptorSchema.safeParse({
      name: 'node-ok',
      gatewayName: 'my-gw',
      interceptionPoints: ['REQUEST'],
      config: { managed: { codeLocation: 'app/node-ok/', runtime: 'nodejs22.x', entrypoint: 'index.handler' } },
    });
    expect(r.success).toBe(true);
  });

  it('accepts a nodejs22.x runtime with no entrypoint (resolved at consumption time)', () => {
    const r = InterceptorSchema.safeParse({
      name: 'node-sparse',
      gatewayName: 'my-gw',
      interceptionPoints: ['REQUEST'],
      config: { managed: { codeLocation: 'app/node-sparse/', runtime: 'nodejs22.x' } },
    });
    expect(r.success).toBe(true);
  });
});
...
describe('resolveInterceptorEntrypoint', () => {
  it('returns an explicit entrypoint unchanged', () => {
    expect(resolveInterceptorEntrypoint({ codeLocation: 'x', entrypoint: 'custom.fn' })).toBe('custom.fn');
  });

  it('defaults a python runtime to handler.lambda_handler', () => {
    expect(resolveInterceptorEntrypoint({ codeLocation: 'x', runtime: 'python3.12' })).toBe('handler.lambda_handler');
  });

  it('defaults a node runtime to index.handler (not the python default)', () => {
    expect(resolveInterceptorEntrypoint({ codeLocation: 'x', runtime: 'nodejs22.x' })).toBe('index.handler');
  });

  it('defaults to python when runtime is absent', () => {
    expect(resolveInterceptorEntrypoint({ codeLocation: 'x' })).toBe('handler.lambda_handler');
  });
});
