...
export class HarnessConfigReader {
  ...
  private async readPrompt(filePath: string, field: string): Promise<string> {
    const file = await open(filePath, "r").catch((error: NodeJS.ErrnoException) => {
      throw Object.assign(new Error(`${field}: cannot read prompt file '${filePath}': ${error.message}`, { cause: error }), { code: error.code });
    });
    try {
