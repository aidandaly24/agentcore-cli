    ...
    if (errorName === 'ResourceNotFoundException') {
      return {
        ...
        error: new ResourceNotFoundError(
          `Log group "${logGroupName}" does not exist. The interceptor Lambda creates it on first invocation — invoke once via \`agentcore invoke interceptor --name ${options.name}\` before tailing logs.`
        ),
      };
    }
