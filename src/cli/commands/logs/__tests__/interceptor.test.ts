...
describe('handleLogsInterceptor', () => {
  ...
  it('maps ResourceNotFoundException to a structured ResourceNotFoundError with invoke remediation', async () => {
    ...
    if (!r.success) {
      ...
      expect(r.error.message).toMatch(/agentcore invoke interceptor --name auth/);
    }
  });
