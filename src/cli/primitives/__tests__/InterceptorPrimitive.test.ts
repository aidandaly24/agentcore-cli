...
const setupNodeProjectMock = vi.fn<(opts: { projectDir: string }) => Promise<{ status: string; error?: string }>>(() =>
...
vi.mock('../../operations/node', () => ({
  setupNodeProject: setupNodeProjectMock,
}));
...
describe('InterceptorPrimitive', () => {
  beforeEach(() => {
    ...
    setupNodeProjectMock.mockReset();
    setupNodeProjectMock.mockResolvedValue({ status: 'success' as const });
  });
  ...
  describe('add', () => {
    ...
    it('runs npm install for a managed Node interceptor so the SDK bundles', async () => {
      mockReadProjectSpec.mockResolvedValue(makeProject());
      setupNodeProjectMock.mockResolvedValueOnce({ status: 'success' as const });
      const result = await primitive.add({
        name: 'node-auth',
        gatewayName: 'my-gw',
        interceptionPoints: ['REQUEST'],
        passRequestHeaders: true,
        config: { managed: { codeLocation: 'app/node-auth/', runtime: 'nodejs22.x' } },
      });
      expect(result.success).toBe(true);
      expect(setupNodeProjectMock).toHaveBeenCalledWith({ projectDir: expect.stringContaining('app/node-auth') });
      if (result.success) expect(result.warning).toBeUndefined();
    });

    it('surfaces a warning (not a failure) when npm install does not complete', async () => {
      mockReadProjectSpec.mockResolvedValue(makeProject());
      setupNodeProjectMock.mockResolvedValueOnce({ status: 'npm_not_found' as const, error: "'npm' not found." });
      const result = await primitive.add({
        name: 'node-auth2',
        gatewayName: 'my-gw',
        interceptionPoints: ['REQUEST'],
        passRequestHeaders: true,
        config: { managed: { codeLocation: 'app/node-auth2/', runtime: 'nodejs22.x' } },
      });
      expect(result.success).toBe(true);
      if (result.success) expect(result.warning).toMatch(/npm install/);
    });

    it('does NOT run npm install for a managed Python interceptor', async () => {
      mockReadProjectSpec.mockResolvedValue(makeProject());
      await primitive.add({
        name: 'py-auth',
        gatewayName: 'my-gw',
        interceptionPoints: ['REQUEST'],
        passRequestHeaders: true,
        config: { managed: { codeLocation: 'app/py-auth/', runtime: 'python3.12' } },
      });
      expect(setupNodeProjectMock).not.toHaveBeenCalled();
    });
